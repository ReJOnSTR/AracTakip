...
export default function PeriodicInspections() {
    ...
    useEffect(() => {
        ...
        const unsub = window.electronAPI?.onDbUpdate?.((change) => {
            if (['inspections', 'vehicles', 'documents'].includes(change?.table)) {
                console.log(`[RealTime] PeriodicInspections reloading for change in ${change.table}`)
    ...
    const loadData = async (isBackground = false) => {
        ...
        try {
            const [inspResult, vehiclesResult, documentsResult] = await Promise.all([
                window.electronAPI.getAllInspections(currentCompany.id, 'periodic', showArchived ? 1 : 0),
    ...
    const handleBulkDeleteClick = (ids) => {
        setConfirmModal({
            ...
            ids: ids,
            title: 'Toplu Silme',
            message: `${ids.length} periyodik kontrol kaydını silmek istediğinize emin misiniz?`
        })
    }
    ...
    const archivedColumns = [
        ...
        {
            key: 'next_inspection',
            label: 'Geçerlilik Bitiş', // Renamed for Archive context
            render: (value) => formatDate(value)
        },
        // 'Kalan Süre' column removed for Archive
        {
            key: 'result',
            label: 'Sonuç',
            render: (value) => {
    ...
    const getDocument = (inspectionId) => {
        const found = documents.find(d => ['periodic_inspection', 'inspection'].includes(d.related_type) && Number(d.related_id) === Number(inspectionId))
        ...
        if (insp?.file_path) {
            return {
                ...
                related_type: 'periodic_inspection',

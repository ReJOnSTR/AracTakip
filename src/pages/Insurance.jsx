...
export default function Insurance() {
    ...
    const handleDelete = async (insurance) => {
        if (confirm('Bu sigorta kaydını silmek istediğinize emin misiniz?')) {
            const result = await window.electronAPI.deleteInsurance(insurance.id)
            ...
            {insurances.length === 0 ? (
                ...
                <DataTable
                    ...
                    onBulkDelete={async (ids) => {
                        if (confirm(`${ids.length} sigorta kaydını silmek istediğinize emin misiniz?`)) {
                            for (const id of ids) {
